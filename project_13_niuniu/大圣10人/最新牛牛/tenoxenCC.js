					...
					if (win.version == '1.0.0') {
						...
						if (parseInt(d.game) === 1 || parseInt(d.game) === 4 || parseInt(d.game) === 8 || parseInt(d.game) === 9 || parseInt(d.game) === 10 || parseInt(d.game) === 11) {
							...
							if (parseInt(d.zhuang_type) === 1) {
								...
								zhuangTypeText = '固定庄家';
							} else if (parseInt(d.zhuang_type) === 6) {
								zhuangTypeText = '明牌撑船';
							} else if (parseInt(d.zhuang_type) === 7) {
								zhuangTypeText = '暗牌撑船';
							} else if (parseInt(d.zhuang_type) === 8) {
								zhuangTypeText = '超级庄家';
							}
							...
							if (win.sign == 'dashengzhongyu' || win.sign == 'shouquandating' || win.sign == 'lianyundating' || win.sign == 'liuliuxianyue' || win.sign == 'yinghuazhongyu' || win.sign == 'shenhuazhongting') {
								if (parseInt(d.max_matches) === 12) {
							...
							if (win.sign == 'dashengzhongyu' || win.sign == 'shouquandating' || win.sign == 'lianyundating' || win.sign == 'liuliuxianyue' || win.sign == 'yinghuazhongyu' || win.sign == 'shenhuazhongting' || win.sign == 'wangzhedating') {
								if (parseInt(d.max_matches) === 12) {
									texaMatches = '12局×3房卡 ';
								} else if (parseInt(d.max_matches) === 24) {
									texaMatches = '24局×6房卡 ';
							...
							joinUser += '<p>限注：' + d.rule.max_point + '</p>';
							var pushMatches = '';
							if (parseInt(d.max_matches) === 12) {
								pushMatches = '12局×3房卡 ';
							} else if (parseInt(d.max_matches) === 24) {
...
function Gold(source, target) {
	...
	var img = new Image();
	img.src = "//cdn-1255620552.file.myqcloud.com/images/niuniu/gold.png";
	img.onload = function() {
		move();
	};
...
$.fn.movescroll = function() {
	if (typeof(this[0].startTop) != 'undefined') return;
	var $this = this;
	var $parent = this.parent();
	$parent[0].startTop = Math.round($this.position().top);
	$parent[0].endTop = Math.round($parent.height() - $this.height());
	$parent.on('mousewheel', function(ev) {
	...
	$parent.on('touchstart', function(ev) {
		...
		$(document.body).on('touchend', function(ev) {
			...
			if ($parent[0]._offset === false) return;
			var top = $parent[0].thisTop;
			var _top = top;
			var startTop = $parent[0].startTop;
			var endTop = $parent[0].endTop;
			...
			if (top != _top) {
				...
				$parent[0]._offset = false;
			} else if (Math.abs($parent[0]._offset) > 5 && isIOS()) {
				var offset = Math.round($parent[0]._offset);
				(function action() {
					if (offset > 0) offset--;
					else offset++;
					...
					if (Math.abs(offset) > 0 && _top == 0 && $parent[0]._offset !== false) {
	...
	$parent.on('touchmove', function(ev) {
		var touch = ev.touches[0];
		var y = Number(touch.pageY);
		if (y < $parent.offset().top || y > $parent.offset().top + $parent.height()) return;
...
function createRanking(data, func) {
	...
	var width = 750;
	var height = 1216;
	var pics = ['//cdn-1255620552.file.myqcloud.com/images/ranking_' + game_id + '_bg.jpg', '//cdn-1255620552.file.myqcloud.com/images/people_bg.png', '//cdn-1255620552.file.myqcloud.com/images/ranking_icon.png'];
	if (users.length > 6) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/people_bg2.jpg');
		pics.push('//cdn-1255620552.file.myqcloud.com/images/people_bg3.jpg');
		height += 102 * (users.length - 6);
	}
	for (var i in users) {
		if (/\/+[064]{1,2}$/.test(users[i].path)) pics.push('//cdn-1255620552.file.myqcloud.com/images/default_head.png');
	...
	for (var i in pics) {
		...
		xhr.onload = function() {
			...
			if (this.status == 200) {
				...
				this.src = '//cdn-1255620552.file.myqcloud.com/images/default_head.png';
			}
		...
		xhr.onerror = function() {
			var img = document.createElement("img");
			img.setAttribute("crossOrigin", 'Anonymous');
			img.src = '//cdn-1255620552.file.myqcloud.com/images/default_head.png';
			imgs[this._index] = img;
			img.onload = function(e) {
				count++;
				if (count >= pics.length) draw();
...
function liuliuCreateRanking(data, func) {
	...
	var width = 750;
	var height = 1216;
	var pics = ['//cdn-1255620552.file.myqcloud.com/images/common/ranking_' + game_id + '_bg.jpg', '//cdn-1255620552.file.myqcloud.com/images/people_bg.png', '//cdn-1255620552.file.myqcloud.com/images/ranking_icon.png'];
	if (users.length > 6) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/common/people_bg2.jpg');
		pics.push('//cdn-1255620552.file.myqcloud.com/images/common/people_bg3.jpg');
		height += 102 * (users.length - 6);
	}
	for (var i in users) {
		if (/\/+[064]{1,2}$/.test(users[i].path)) pics.push('//cdn-1255620552.file.myqcloud.com/images/default_head.png');
...
function createRankingV2(data, func) {
	...
	var width = 750;
	var height = 1216;
	var pics = ['//cdn-1255620552.file.myqcloud.com/images/decIndex/ranking_bg.jpg', '//cdn-1255620552.file.myqcloud.com/images/decIndex/people_bg.png?v=1', '//cdn-1255620552.file.myqcloud.com/images/decIndex/ranking_icon.png'];
	if (users.length > 6) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/decIndex/people_bg2.jpg');
		pics.push('//cdn-1255620552.file.myqcloud.com/images/decIndex/people_bg3.jpg');
		height += 102 * (users.length - 6);
	}
	for (var i in users) {
		if (/\/\/[064]{1,2}$/.test(users[i].path)) pics.push('//cdn-1255620552.file.myqcloud.com/images/default_head.png');
...
function canvasRanking(data, func) {
	...
	var can = $canvas.get(0).getContext("2d");
	var str = ["//cdn-1255620552.file.myqcloud.com/images/bull/rank_bg.jpg", "//cdn-1255620552.file.myqcloud.com/images/bull/rank_frame62.png", '//cdn-1255620552.file.myqcloud.com/images/bull/scoresRank3.png', '//cdn-1255620552.file.myqcloud.com/images/bull/rank_bigwinner2.png', '//cdn-1255620552.file.myqcloud.com/images/bull/score_search1.png'];
	var index = 0,
		arr = [];
	for (var i in str) {
		if (XMLHttpRequest)
...
function canvasQRCodeCreate(data, codeUrl) {
	...
	var pics = [];
	if (parseInt(game_id) === 1) {
		pics.push("//cdn-1255620552.file.myqcloud.com/images/game-niuniu.jpg");
		gameName = '牛.牛';
	} else if (parseInt(game_id) === 2) {
		pics.push("//cdn-1255620552.file.myqcloud.com/images/game-jinhua.jpg");
		gameName = '金花';
	} else if (parseInt(game_id) === 3) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/thirteencards/game_logo.png');
		gameName = '十三张';
	} else if (parseInt(game_id) === 4) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/niuniuten/share-niuniuten.jpg');
		gameName = '十人牛.牛';
	} else if (parseInt(game_id) === 5) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/texasholdem/game-texasholdem.jpg');
		gameName = '德州';
	} else if (parseInt(game_id) === 6) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/sangong/share-sangong.jpg');
		gameName = '三公';
	} else if (parseInt(game_id) === 8) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/liuliuxianyue/bull6-share.jpg');
		gameName = '六人牛.牛';
	} else if (parseInt(game_id) === 8) {
		pics.push('//cdn-1255620552.file.myqcloud.com/images/liuliuxianyue/bull9-share.jpg');
		gameName = '九人牛.牛';
	}
	var index = 0;
	var imgs = [];
...
function getShareIconLink(game_id) {
	...
	if (gameVersionJson[win.sign] == undefined || shareLinkJson[game_id] == undefined) {
		return ['http:', 'https:'][IS_SSL] + '//cdn-1255620552.file.myqcloud.com/images/shareIcon/undefined-share-image.jpg';
	} else {
		return ['http:', 'https:'][IS_SSL] + '//cdn-1255620552.file.myqcloud.com/images/shareIcon/' + 'v' + gameVersionJson[win.sign] + '/' + 'share-' + shareLinkJson[game_id] + '.jpg';
	}
}
var notice = {
	data: '',
	...
	show: function(content) {
		var em = document.querySelector('.noticeBtn');
		if (em) em.parentNode.removeChild(em);
		var em = document.querySelector('.noticeBg');
		if (em) em.parentNode.removeChild(em);
		var code = '<a href="javascript:void(0);" class="noticeBtn"></a><div class="noticeBg"><div class="noticeBox"><a href="javascript:void(0);" class="closeBtn"></a><div class="content"><div class="content2">' + content + '</div></div><a href="javascript:void(0);" class="tail">本次公告不再显示</a></div></div>';
		document.getElementsByTagName('body')[0].insertAdjacentHTML("beforeend", code);
		var btn = document.querySelector('.noticeBtn');
		var noticeEm = document.querySelector('.noticeBg');
		var localPos = {
			x: 0,
			y: 0,
			left: parseInt(btn.offsetLeft),
			top: parseInt(btn.offsetTop)
		};
		var _data = {
			left: localPos.left,
			top: localPos.top,
		...
		if (storage.get('noticeLocalData')) {
			_data = JSON.parse(storage.get('noticeLocalData'));
			localPos.left = _data.left;
			localPos.top = _data.top;
		}
		...
		if (_data.firstOpen == 1) btn.classList.add('new');
		if (_data.display == 0) {
			btn.parentNode.removeChild(btn);
			noticeEm.parentNode.removeChild(noticeEm);
			return;
		}
		storage.set('noticeLocalData', JSON.stringify(_data));
		var startPos = localPos;
		btn.style.left = startPos.left + 'px';
		btn.style.top = startPos.top + 'px';
		var time = 0;
		...
		btn.addEventListener('touchmove', function(ev) {
			...
			var offsetX = pos.x - startPos.x + startPos.left;
			var offsetY = pos.y - startPos.y + startPos.top;
			if (offsetX > 0 && offsetX < win.width - parseInt(btn.offsetWidth)) {
				btn.style.left = offsetX + 'px';
				startPos.x = pos.x;
				startPos.left = offsetX;
			}
			if (offsetY > 0 && offsetY < win.height - parseInt(btn.offsetHeight)) {
				btn.style.top = offsetY + 'px';
				startPos.y = pos.y;
				startPos.top = offsetY;
			}
			localPos = startPos;
		}, false);
		btn.addEventListener('touchend', function(ev) {
			if ((new Date()).getTime() - time < 200 && noticeEm.style.display != 'block' && Math.abs(_data.left - localPos.left) < 5 && Math.abs(_data.top - localPos.top) < 5) {
				noticeEm.style.display = 'block';
				this.classList.remove('new');
				_data.firstOpen = 0;
				if (typeof(jQuery) == 'function') {
					$('.noticeBox .content').css('overflow-y', 'hidden');
					$('.noticeBox .content2').movescroll();
				}
			}
			_data.left = localPos.left;
			_data.top = localPos.top;
			storage.set('noticeLocalData', JSON.stringify(_data));
		}, false);
		document.querySelector('.noticeBox .closeBtn').onclick = function() {
			this.parentNode.parentNode.style.display = 'none';
			if (document.querySelector('.noticeBg .choose')) {
				btn.parentNode.removeChild(btn);
				var data = JSON.parse(storage.get('noticeLocalData'));
				data.display = 0;
				storage.set('noticeLocalData', JSON.stringify(data));
			}
		};
		document.querySelector('.noticeBg .tail').onclick = function() {
			if (this.classList.value.indexOf('choose') == -1) {
	...
	run2: function() {
		...
		var datas = [];
		try {
			datas = JSON.parse(Base64.decode(this.content));
		} catch (e) {
			return;
		}
		var time = Math.round((new Date()).getTime() / 1000);
		if ((datas.play_time <= time && datas.end_time >= time) || (datas.play_time <= time && datas.end_time == 0)) {
			this.show(datas.contents);
		} else {
			this.hide();
		}
	},
...
function app_command(act, data, fun) {
	setupWebViewJavascriptBridge(function(bridge) {
		if (typeof(data) == 'function') {
			...
			bridge.registerHandler(act, function(_data, responseCallback) {
			...
			data = data || {};
			bridge.callHandler(act, data, function(response) {
...
var Page = {
	...
	load: function() {
		...
		win.readyJoin(Page.code, function() {
			ws.connect('home/douniuTencc', function() {
...
var game = {
	...
	init: function(d) {
		...
		if (d.limit > 0) {
			$('<div class="limit-number">庄家限额:' + d.limit + '</div>').appendTo('.body');
		}
		if (d.next_master) {
		...
		if (d.info) {
			...
			if (d.zhuang_type == 1) {
				...
				var typeText = "固定庄家";
				$('.player-number').html(playerNumber);
			} else if (d.zhuang_type == 6) {
				var typeText = "明牌撑船";
			} else if (d.zhuang_type == 7) {
				var typeText = "暗牌撑船";
			} else if (d.zhuang_type == 8) {
				var typeText = "超级庄家";
			}
			...
			if (d.zhuang_type == 5) {
				...
				$('.rules-mask .flex-cont[data-pos="5"]').show();
				$('.rules-mask .flex-cont[data-pos="5"] .name').html('限额：');
				$('.rules-mask .flex-cont[data-pos="5"] .flex-item').html('<span>' + d.limit_value + '</span>');
			} else {
			...
			if (win.app > 0) {
				app_command('share', function() {
					return WEB_TITLE + '：十人牛' + '\n\r' + '模式:' + typeText + ' 底分:' + d.end_points + '分 规则:' + card_rule + ' 局数:' + matchesText + ' 牌型:' + shareValueText + (d.zhuang_type == 5 ? '上庄:' + d.zhuang_value : '') + (d.zhuang_type == 8 ? '限额:' + d.limit_value : '') + '\n\r房间ID：' + d.number + '\n\r房间链接：' + ['http://', 'https://'][IS_SSL] + DOMAIN + '/tenoxen.html?code=' + Page.code;
				});
			} else {
				Page.shareData = {
					...
					link: location.href,
					path: '//cdn-1255620552.file.myqcloud.com/images/niuniuten/share-niuniuten.jpg'
				};
			}
				...
				for (var i in user_list) {
					...
					switch (parseInt(d.running)) {
						...
						case 2:
							...
							if (d.zhuang_type == 6) {
								$('.tips-text').remove();
								$('<div class="tips-text">').html('选倍').appendTo('.body');
								$('.player-number').show();
							}
							if (d.zhuang_type == 8) {
								if (d.limits && d.limits.length > 0) {
									$('.rob-zhuang-num .item').show();
									if (user_list[i].zhuang_limit) {
										if (user_list[i].zhuang_limit == 0) {
											$('.rob-zhuang-num .item[data-pos="' + player.pos + '"]').html('<span>不抢</span>');
										} else {
											$('.rob-zhuang-num .item[data-pos="' + player.pos + '"]').html('<span>' + user_list[i].zhuang_limit + '</span>');
										}
									}
									if (myuser.zhuang_limit) {
										$('.grabzhuang').hide();
									} else {
										$('.tips-text').remove();
										$('<div class="tips-text">').html('庄家限额抢庄').appendTo('.body');
										var code = '<input type="button" data-item="9" data-value="' + d.limits[0] + '" value="' + d.limits[0] + '"><input type="button" data-item="9" data-value="' + d.limits[1] + '" value="' + d.limits[1] + '"><input type="button" data-item="9" data-value="' + d.limits[2] + '" value="' + d.limits[2] + '"><input type="button" data-item="10" data-value="0" name="0" value="不抢">';
										$('.grabzhuang').html(code).show();
										$('.grabzhuang input[data-value="' + d.limits[0] + '"]').touch(function() {
											game.clickgrabZhuang(d.limits[0]);
										});
										$('.grabzhuang input[data-value="' + d.limits[1] + '"]').touch(function() {
											game.clickgrabZhuang(d.limits[1]);
										});
										$('.grabzhuang input[data-value="' + d.limits[2] + '"]').touch(function() {
											game.clickgrabZhuang(d.limits[2]);
										});
										$('.grabzhuang input[data-value="0"]').touch(function() {
											game.clickgrabZhuang('0');
										});
						...
						case 4:
							...
							if (d.zhuang_type == 4 || d.zhuang_type == 5 || d.zhuang_type == 8) {
								$('.player-number').hide();
								$('.showdown').show();
								$('.lookCardText').hide();
								if (user_list[0].code) {
	...
	start: function(data) {
		...
		if (game.zhuang_type == 8) {
			if (data.limits && data.limits.length > 0) {
				$('.tips-text').remove();
				$('<div class="tips-text">').html('庄家限额抢庄').appendTo('.body');
				var code = '<input type="button" data-item="9" data-value="' + data.limits[0] + '" value="' + data.limits[0] + '"><input type="button" data-item="9" data-value="' + data.limits[1] + '" value="' + data.limits[1] + '"><input type="button" data-item="9" data-value="' + data.limits[2] + '" value="' + data.limits[2] + '"><input type="button" data-item="10" data-value="0" name="0" value="不抢">';
				setTimeout(function() {
					$('.grabzhuang').html(code).show();
					$('.grabzhuang input[data-value="' + data.limits[0] + '"]').touch(function() {
						game.clickgrabZhuang(data.limits[0]);
					});
					$('.grabzhuang input[data-value="' + data.limits[1] + '"]').touch(function() {
						game.clickgrabZhuang(data.limits[1]);
					});
					$('.grabzhuang input[data-value="' + data.limits[2] + '"]').touch(function() {
						game.clickgrabZhuang(data.limits[2]);
					});
					$('.grabzhuang input[data-value="0"]').touch(function() {
						game.clickgrabZhuang('0');
					});
	...
	grabZhuang: function(data) {
		...
		if (game.zhuang_type == 8) {
			console.log(data.zhuang_multiple);
			if (data.zhuang_multiple == 0) {
				$('.rob-zhuang-num .item[data-pos="' + pos + '"]').html('<span>不抢</span>');
				sound.play(99, data.sex);
			} else {
	...
	selectedMaster: function(data) {
		...
		if (!data.random_users) {
			setTimeout(function() {
				if (data.limit) {
					game.limitNumber = data.limit;
					$('<div class="limit-number">庄家限额:' + data.limit + '</div>').appendTo('.body');
				}
				$('.rob-zhuang-num .item').hide();
	...
	setAnte: function(data) {
		...
		$('.selectMultiple .item').show();
		var pos = $('.user-info[data-id="' + data.user_id + '"]').data('pos');
		$('.selectMultiple .item[data-pos="' + pos + '"]').html('<span>×' + data.chip + '</span>');
	...
	showMultiple: function(data) {
		...
		$('<div class="tips-text" >').html('等待摊牌').appendTo('.body');
		var roomType = $('.zhuangType').attr('data-item');
		if (game.zhuang_type == 1 || game.zhuang_type == 6) {
			$('.user-card[data-pos="0"] .cardbox[data-item="4"]').touch(function() {
				game.clickTurnover();
			});
		}
		if (roomType == 3) {
			...
			$('.user-card[data-pos="0"] .cardbox[data-item="4"]').on('click', function() {
				$(this).removeClass('hind').addClass('front');
			});
			$('.showdown').show();
			$('.lookCardText').hide();
		...
		if (roomType == 4 || roomType == 5) {
			...
			$('.user-card[data-pos="0"] .cardbox[data-item="3"]').on('click', function() {
				$(this).removeClass('hind').addClass('front');
			});
			$('.user-card[data-pos="0"] .cardbox[data-item="4"]').on('click', function() {
				$(this).removeClass('hind').addClass('front');
	...
	settlement: function(data) {
		...
		if (game.zhuang_type == 6) {
			...
			for (var i = data.length - 1; i >= 0; i--) {
				if (data[i].compensate_to.length > 0) {
					for (var j in data[i].compensate_to) {
						var b = {};
						b[data[i].user_id] = [data[i].compensate_to[j]];
						userGoldArr.push(b);
					}
				}
			}
			for (var k in userGoldArr) {
				(function goldFly(k, data) {
					setTimeout(function() {
						for (var m in data) {
							Gold2(m, data[m]);
						}
						if (k == userGoldArr.length - 1) {
...
function Player(user_id, nickname, path, is_join, value) {
	...
	this.sendCard = function(cards, n) {
		...
		if (game.zhuang_type == 6) {
			for (var i = 0; i < n; i++) {
				if (cards.length > i) {
					$('.user-card[data-pos="' + this.pos + '"] span.card:eq(' + i + ')').attr({
						'data-value': cards[i].value,
						'data-color': cards[i].color
					});
					$('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front');
