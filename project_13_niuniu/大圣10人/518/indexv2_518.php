...
var Page = {
        ...
        ajax('nine9ye/create10NiuRoom', data, function(d) { if (d.status == 1) { $('.niuniuTen-mask').hide(); if (USE_QRCODE) { var url = 'http://' + JUMP_DOMAIN + '/tenniuniu?type=518&room_code=' + d.info.code;
                    qrcodeCreate(url, 4, generalQrcodeData(d.info)); if (parseInt(d.info.count_matchs) === 12) { $('.user-info .room-card span').text(Page.roomCard - 2);
                        ...
                        Page.roomCard = parseInt($('.user-info .room-card span').text()); } } else { 
                            ...
                            location.href = 'tenniuniu?type=518&room_code=' + d.info.code; } } else if (d.status == 2) { $.alert('已有创建房间', 'error'); } else if (d.status == 3) { Page.phoneMask('.niuniuTen-mask'); } else { $.alert("请联系代理商！"); } }); },
    ...
    fillingPitCardSubmit: function() {
        ...
        ajax('home/gameFillingpit/createRoom', data, function(d) { if (d.status == 1) { location.href = 'fillingPit.html?code=' + d.info; } else if (d.status == 3) { Page.phoneMask('.fillingPitcard-mask'); } else { $.alert("请联系代理商！"); } });
    },
