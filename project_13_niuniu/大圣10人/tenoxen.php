...
var Page = {
    ...
    load: function() {
        ...
        $('.bottom .chat').click(function() { $('.chat-list-mask').show();
            $('.chat-list').show(); });
        $('.bottom .chat-list-mask').click(function() { $('.chat-list-mask').hide();
            $('.chat-list').hide(); });
        $('.bottom .chat-list li').click(function() { $('.chat-list-mask').hide();
            $('.chat-list').hide(); var chat = $(this).attr('data-item');
    ...
    join: function() { $('.join-user').remove();
        Page.init(); },
    init: function() { $('.prepare-item').hide();
        $('.prepare').hide();
        $('.tips-text').remove();
        $('.grabzhuang').hide();
        $('.player-number').hide();
    ...
    wsCallback: function() { ws.callback.init = function(data) { game.init(data); };
    ...
    playerPosition: function(pos1, pos2) { var arr = [9, 5, 4, 7, 3, 6, 2, 1, 8, 0, 9, 5, 4, 7, 3, 6, 2, 1, 8]; var myPos = 9; var playerNum = 0;
        playerNum = myPos + (pos2 - pos1); return arr[playerNum]; },
};
...
var game = {
    ...
    init: function(d) {
        ...
        if (d.info) { if (d.info == 0) { $.alert('加入房间失败', function() { wx.closeWindow(); }) } else if (d.info == -1) { $.alert('房间号错误', function() { wx.closeWindow(); }) } else if (d.info == 1) { $('body').css({ 'background': 'none', 'min-height': 'initial' });
                $('.body').remove();
                ajax('home/index/result', { code: Page.code }, function(data) { if (data == 'error') { Page.init(); } else { createRanking(data, function(data) { var $img = $('<img class="room-gameover">').attr('src', data);
                            $img.appendTo('body').show();
                            $(document.body).off('touchmove');
                            $('#loadings').fadeOut('fast', function() { $(this).remove(); }); }) } }); } else if (d.info == 2) { $.alert('该房间人数已满', function() { wx.closeWindow(); }) } } else {
            ...
            if (d.zhuang_type == 1) { var typeText = "明牌抢庄"; } else if (d.zhuang_type == 2) { var typeText = "通比牛.牛"; } else if (d.zhuang_type == 3) { var typeText = "自由抢庄";
                $('.player-number').html(playerNumber); } else if (d.zhuang_type == 4) { var typeText = "牛.牛上庄";
                $('.player-number').html(playerNumber); } else if (d.zhuang_type == 5) { var typeText = "固定庄家";
            ...
            $('.prepare').touch(function() { game.clickReady(); });
            $('.showdown').touch(function() { game.clickShowdown(); });
            if (d.zhuang_type == 5) { $('.rules-mask .flex-cont[data-pos="5"]').show();
                $('.rules-mask .flex-cont[data-pos="5"] .flex-item').html('<span>' + d.zhuang_value + '</span>'); } else { $('.rules-mask .flex-cont[data-pos="5"]').hide(); }
            ...
            var shareValueText = '';
            for (num in arrValue) {
                if (arrValue[num] == 1) { arrValueText += '<span>五花牛（5倍）</span>';
                    shareValueText += '五花牛（5倍）'; }
                if (arrValue[num] == 2) { arrValueText += '<span>炸弹牛（6倍）</span>';
                    shareValueText += '炸弹牛（6倍）'; }
                if (arrValue[num] == 3) { arrValueText += '<span>五小牛（8倍）</span>';
                    shareValueText += '五小牛（8倍）'; }
                if (arrValue[0] == '') { arrValueText += '<span>牛.牛</span>';
                    shareValueText += '牛.牛'; }
            }
                    ...
                    switch (parseInt(d.running)) {
                        case 0:
                            ...
                            if (user_list[i].ready || user_list[i].ready == 1) { $('.prepare-item[data-pos="' + player.pos + '"]').show();
                                $('.tips-text').remove(); }
                            if (user_list[0].ready || user_list[0].ready == 1) { $('.prepare').hide();
                                $('.prepare-item[data-pos="0"]').show(); } else { $('.prepare').show(); }
                        ...
                        case 2:
                            ...
                            if (d.zhuang_type == 1) {
                                ...
                                setTimeout(function() { $('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front');
                                    $('.user-card[data-pos="0"] .cardbox[data-item="4"]').removeClass('front').addClass('hind'); }, 600);
                            ...
                            if (d.zhuang_type == 3) {
                                $('.rob-zhuang-num .item').show();
                                if (!myuser.zhuang_multiple) { var btn = '<div class="zhuang_btn" >';
                            ...
                            if (d.zhuang_type == 2) { $('.tips-text').remove();
                                $('.grabzhuang').hide();
                                $('.showdown').show(); }
                            if (d.zhuang_type == 4 || d.zhuang_type == 5) { $('.tips-text').remove();
                                $('.grabzhuang').hide(); }
                            if (!user_list[0].is_join || user_list[0].is_join == 0) { $('.grabzhuang').hide();
                                $('.zhuang_btn').hide();
                        ...
                        case 3:
                            ...
                            if (user_list[i].is_master == 1) { $('.user-info[data-id="' + user_list[i].user_id + '"]').addClass('choosed');
                                $('.banker span').hide();
                            ...
                            if (user_list[i].multiple && !user_list[i].is_master) { $('.selectMultiple .item[data-pos="' + player.pos + '"]').html('<span>×' + user_list[i].multiple + '</span>');
                                $('.selectMultiple .item[data-pos="' + player.pos + '"]').show(); }
                            if (user_list[0].multiple) { $('.player-number').hide(); } else { $('.player-number').show(); }
                            if (user_list[0].is_master == 1 && user_list[0].multiple) { $('.grabzhuang').hide();
                                $('.player-number').hide(); }
                            player.sendCard(user_list[i].cards);
                            if (!user_list[i].is_join || user_list[i].is_join == 0) { $('.user-card[data-pos="' + player.pos + '"]').hide(); }
                            if (d.zhuang_type == 1) { setTimeout(function() { $('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front');
                                    $('.user-card[data-pos="0"] .cardbox[data-item="4"]').removeClass('front').addClass('hind'); }, 600); var cards = user_list[0].cards; if (cards && cards.length > 0) { for (j in cards) { $('.user-card[data-pos="0"] .cardbox[data-item="' + j + '"] span.card').attr({ 'data-value': cards[j].value, 'data-color': cards[j].color }); } } }
                            if (d.zhuang_type == 3) { $('.zhuang_btn').remove();
                                $('.grabzhuang').hide(); }
                        ...
                        case 4:
                            ...
                            if (user_list[i].is_master == 1) { $('.user-info[data-id="' + user_list[i].user_id + '"]').addClass('choosed');
                                $('.banker span').hide();
                                $('.banker span[data-pos="' + player.pos + '"]').show(); };
                            if (user_list[i].multiple) { $('.selectMultiple .item').show();
                                $('.selectMultiple .item[data-pos="' + player.pos + '"]').empty();
                            ...
                            if (d.zhuang_type == 1) {
                                var cards = user_list[0].cards;
                                if (cards && cards.length > 0) { for (j in cards) { $('.user-card[data-pos="0"] .cardbox[data-item="' + j + '"] span.card').attr({ 'data-value': cards[j].value, 'data-color': cards[j].color }); } }
                                if (myuser.code) { $('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front'); } else { $('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front');
                            ...
                            if (d.zhuang_type == 2) {
                                ...
                                $('.lookCardText').hide();
                                $('.selectMultiple .item[data-pos="' + player.pos + '"]').empty();
                                if (user_list[0].code) { $('.showdown').hide();
                                    $('.user-card[data-pos="0"] >div[data-item="3"]').removeClass('hind');
                                    $('.user-card[data-pos="0"] >div[data-item="4"]').removeClass('hind'); } else { $('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front');
                            ...
                            if (user_list[0].code) { $('.lookCardText').hide();
                                $('.showdown').hide(); }
                            if (!user_list[0].is_join || user_list[0].is_join == 0) { $('.player-number').hide();
    ...
    join: function(data) {
        if (!players['u_' + data.user_id]) {
            ...
            Page.userLength++;
            if ($('.user-info').length != Page.userLength) { Page.init() }
        } else { players['u_' + data.user_id].changeStatus(1);
            $('.user-info[data-id="' + data.user_id + '"]').removeClass('leave'); }
    },
    leave: function(data) { if (data.is_exist == 1) { players['u_' + data.user_id].changeStatus(0); } else { Page.readyNum = 0; var pos = $('.user-info[data-id="' + data.user_id + '"]').data('pos');
    ...
    clickReady: function() {
        ...
        if (roomType == 1 || roomType == 2 || roomType == 3) { $('.banker span').hide();
    ...
    clickLeaveRoom: function() { ws.send('', 'leaveRoom');
        wx.closeWindow(); },
    ...
    start: function(data) {
        ...
        setTimeout(function() { sound.play('sound_101');
            $('.user-card').addClass('animate'); }, 500);
        var roomType = $('.zhuangType').attr('data-item');
        if (roomType == 1 || roomType == 2 || roomType == 3) { $('.banker span').hide();
            $('.user-info ').removeClass('choosed'); }
        if (roomType == 1) {
            ...
            if (cards && cards.length > 0) {
                ...
                setTimeout(function() { $('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front');
                    $('.user-card[data-pos="0"] .cardbox[data-item="4"]').removeClass('front').addClass('hind'); }, 1500);
            }
        }
        if (roomType == 2) { $('.prepare').hide();
            setTimeout(function() { $('.grabzhuang').hide();
                $('.showdown').show(); }, 2000);
            $('.tips-text').remove();
            $('.user-card[data-pos="0"] .cardbox[data-item="3"]').addClass('hind');
            setTimeout(function() { $('.user-card[data-pos="0"] .cardbox[data-item="3"]').click(function() { $(this).removeClass('hind').addClass('front'); });
                $('.user-card[data-pos="0"] .cardbox[data-item="4"]').click(function() { $(this).removeClass('hind').addClass('front'); }); }, 2000); }
        if (roomType == 3) { setTimeout(function() { $('.grabzhuang').hide(); var btn = '<div class="zhuang_btn" >';
                btn += '    <div class="upZhuang" onclick="game.clickgrabZhuang(1)">抢庄</div>';
                btn += '    <div class="upZhuang no" onclick="game.clickgrabZhuang(0)")>不抢</div>';
                btn += '</div>';
                $(btn).appendTo('.body'); }, 2000); }
        if (roomType == 4) { $('.tips-text').remove(); }
        if (roomType == 5) { $('.tips-text').remove();
            setTimeout(function() { $('.grabzhuang').hide(); if (user.id == Page.next_master) { $('.player-number').hide();
                    $('.tips-text').remove();
                    $('<div class="tips-text">').html('等待闲家下注').appendTo('.body'); } else { $('.player-number').show();
                    $('<div class="tips-text">').html('闲家下注').appendTo('.body'); } }, 2000) }
        if (players['u_' + user.id].isjoin == 0 || !players['u_' + user.id].isjoin) { var pos = $('.user-info[data-id="' + players['u_' + user.id] + '"]').data('pos');
    ...
    grabZhuang: function(data) {
        ...
        var roomType = $('.zhuangType').attr('data-item');
        if (roomType == 1) { if (data.zhuang_multiple == 0) { $('.rob-zhuang-num .item[data-pos="' + pos + '"]').html('<span>不抢</span>');
                sound.play(99, data.sex); } else { $('.rob-zhuang-num .item[data-pos="' + pos + '"]').html('<span>×' + data.zhuang_multiple + '</span>');
                sound.play(98, data.sex); } }
        if (roomType == 3) { if (data.zhuang_multiple == 0) { $('.rob-zhuang-num .item[data-pos="' + pos + '"]').html('<span>不抢</span>');
                sound.play(99, data.sex); } else { $('.rob-zhuang-num .item[data-pos="' + pos + '"]').html('<span>抢庄</span>');
                sound.play(98, data.sex); } }
    },
    selectedMaster: function(data) {
        ...
        if (!data.random_users) {
            setTimeout(function() {
                ...
                if (data.multiple == 1) { $('.rob-zhuang-num .item[data-pos="' + pos + '"]').empty();
                ...
                setTimeout(function() {
                    ...
                    $('.tips-text').remove();
                    $('<div class="tips-text">').html('闲家下注').appendTo('.body');
                    if (players['u_' + user.id].isjoin == 0 || !players['u_' + user.id].isjoin) { $('.player-number').hide();
                        $('.lookCardText').hide(); }
                }, 500)
            }, 500)
        } else {
    ...
    showMultiple: function(data) {
        ...
        $('.player-number').hide();
        $('.selectMultiple .item').show();
        for (i in data) { var pos = $('.user-info[data-id="' + data[i].user_id + '"]').data('pos');
            $('.selectMultiple .item[data-pos="' + pos + '"]').empty();
            $('.selectMultiple .item[data-pos="' + pos + '"]').html('<span>×' + data[i].multiple + '</span>'); if (data[i].no_choice) { sound.play('x' + data[i].multiple, data[i].sex); } }
        $('.lookCardText').show();
        $('.tips-text').remove();
        $('<div class="tips-text" >').html('等待摊牌').appendTo('.body');
        var roomType = $('.zhuangType').attr('data-item');
        if (game.zhuang_type == 1) { $('.user-card[data-pos="0"] .cardbox[data-item="4"]').touch(function() { game.clickTurnover(); }); }
        if (roomType == 3) { $('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front');
            $('.user-card[data-pos="0"] .cardbox[data-item="3"]').removeClass('front').addClass('hind');
            $('.user-card[data-pos="0"] .cardbox[data-item="4"]').removeClass('front').addClass('hind');
            $('.user-card[data-pos="0"] .cardbox[data-item="3"]').click(function() { $(this).removeClass('hind').addClass('front'); });
            ...
            $('.showdown').show();
            $('.lookCardText').hide(); }
        if (roomType == 4 || roomType == 5) { $('.showdown').show();
            $('.lookCardText').hide();
            setTimeout(function() { $('.user-card[data-pos="0"] .cardbox').removeClass('hind').addClass('front');
                $('.user-card[data-pos="0"] .cardbox[data-item="3"]').removeClass('front').addClass('hind');
                $('.user-card[data-pos="0"] .cardbox[data-item="4"]').removeClass('front').addClass('hind'); }, 300);
            $('.user-card[data-pos="0"] .cardbox[data-item="3"]').on('click', function() { $(this).removeClass('hind').addClass('front'); });
            $('.user-card[data-pos="0"] .cardbox[data-item="4"]').on('click', function() { $(this).removeClass('hind').addClass('front'); }); }
        if (players['u_' + user.id].isjoin == 0 || !players['u_' + user.id].isjoin) { $('.lookCardText').hide();
            $('.selectMultiple .item[data-pos="' + players['u_' + user.id] + '"]').empty();
            $('.showdown').hide();
            $('.tips-text').remove(); }
    },
    clickTurnover: function() { ws.send('', 'turnover'); },
    turnover: function(data) { Page.running = 4; if (this.showdown_users.indexOf(user.id) > -1) return;
        $('.lookCardText').hide();
    ...
    showdown: function(data) {
        ...
        if (data.user_id == user.id) { $('.grabzhuang').hide();
            $('.player-number').hide();
            $('.showdown').hide(); }
    ...
    settlement: function(data) {
        ...
        var userlost = [];
        var userwin = [];
        if (game.lost_value > 0 && game.get_value > 0) { setTimeout(getValue, 1000);
            setTimeout(lostValue, 3000);
            setTimeout(score, 4000);
            setTimeout(resetGame, 7500); } else if (game.lost_value > 0) { setTimeout(lostValue, 1000);
            setTimeout(score, 1000);
            setTimeout(resetGame, 4500); } else if (game.get_value > 0) { setTimeout(getValue, 1000);
            setTimeout(score, 1000);
        ...
        function score() {
            for (var i in data) {
                ...
                setTimeout(function() { $('.winLost').show();
                    setTimeout(function() { $('.winLost').css('top', '-45%');
                        $('.user-info[data-pos="3"] .winLost').css('top', '0%');
                        setTimeout(function() { $('.winLost').remove(); if ($('.canvas_gold')) { $('.canvas_gold').remove(); } }, 2000) }, 200); }, 1000);
            }
        }
        ...
        function resetGame() {
            ...
            var roomType = $('.zhuangType').attr('data-item');
            if (roomType == 5) { if ($('.top .flex-item span:first').text() == 3 || $('.top .flex-item span:first').text() == 6 || $('.top .flex-item span:first').text() == 9) { $('.prepare').hide() } else { $('.prepare').show();
                    $('.time').hide(); } } else { $('.prepare').show();
                $('.time').hide(); }
            $('.tips-text').remove();
            $('.player-number').hide();
            $('<div class="tips-text" >').html('点击准备开始下一局').appendTo('.body');
            $('.showdown').hide();
            if ($('.top .flex-item span:first').text() == $('.top .flex-item span:last').text()) { $('.tips-text').remove();
                $('.prepare').hide();
        ...
        if (data == 'error') { Page.init(); } else {
            if (win.version == '1.0.0') {
                createRanking(data, function(d) {
                    ...
                    img.src = d;
                    if (parseInt(data.users.length) > 6) { img.className = 'room-gameover-ten'; } else { img.className = 'room-gameover'; }
                    img.onload = function() { setTimeout(function() { document.body.appendChild(img);
                ...
                liuliuCreateRanking(data, function(d) {
                    ...
                    if (parseInt(data.users.length) > 6) { img.className = 'room-gameover-ten ranking-img'; } else { img.className = 'room-gameover ranking-img'; }
                    img.src = d;
                    img.onload = function() { setTimeout(function() { document.body.style.backgroundColor = '#000000';
                            document.body.style.minHeight = 'initial';
                            document.body.appendChild(img); var div = document.createElement('div');
                            div.className = 'search-number-box';
                            document.body.appendChild(div); var detailedBtn = '<a class="search-number-box-btn" href="pkdetailed?room_code=' + Page.code + '" style="position: absolute;"></a>';
    ...
    nextZhuang: function(data) { Page.running = 5;
        setTimeout(function() { $('.user-info').removeClass('choosed');
            $('.user-info[data-id=' + data + ']').addClass('choosed');
            $('.banker span').hide(); var pos = $('.user-info[data-id="' + data + '"]').attr('data-pos');
            $('.banker span[data-pos="' + pos + '"]').show(); }, 4000) },
    isXiaZhuang: function(data) {
        ...
        function isZhuang() { if (data == user.id) { var btn = '<div class="zhuang_btn" >';
                ...
                $(btn).appendTo('.body');
                $('.prepare').hide();
                $('.tips-text').remove(); } else { $('.prepare').hide();
                $('.tips-text').remove();
                $('<div class="tips-text" >').css('top', '2.0256rem').html('等待庄家是否下庄').appendTo('.body'); } }
    },
    clickXiaZhuang: function(value) { if (value == 1) { $.dialog('是否下庄并结算本房间？', function() { ws.send(value, 'xiaZhuang') }, 1); } else { ws.send('0', 'xiaZhuang');
            $('.zhuang_btn').remove(); } },
...
function Player(user_id, nickname, path, is_join, value) {
    ...
    if ($('.user-info').length == 0 || is_join == 0) { this.pos = $('.user-info').length; var code = this.getUserDom();
        $(code).appendTo('.user-list'); } else { for (i = 0; i <= $('.user-info').length; i++) { if ($('.user-info[data-pos="' + i + '"]').length == 0) { this.pos = i; var code = this.getUserDom();
                $($('.user-info')[i - 1]).after($(code)); break; } } }
...
const DOMAIN = 'bds.asozu.cn';
const API_DOMAIN = 'bds.asozu.cn';
const WS_DOMAIN = '111.230.23.129:8084';
const JUMP_DOMAIN = 'bds.asozu.cn';
const USE_QRCODE = 0;
const IS_SSL = false;
win.version = '2.0.0';
win.sign = 'shouquandating';
win.token = 'cm1scnF1ODNhb2UybWg0dG45dmlla2gwdm4=';
notice.data = '';
var user = {
    "id": "206116",
    "wechat_id": "1",
...
sound.source.bgm = "http://static.wanzj.cn/music/public/bgm.mp3";
sound.source.gold = "http://static.wanzj.cn/music/public/gold.mp3";
sound.source.sound_1_0 = "http://static.wanzj.cn/music/voice/sound_1_0.mp3";
sound.source.sound_1_1 = "http://static.wanzj.cn/music/voice/sound_1_1.mp3";
sound.source.sound_1_10 = "http://static.wanzj.cn/music/voice/sound_1_10.mp3";
sound.source.sound_1_11 = "http://static.wanzj.cn/music/voice/sound_1_11.mp3";
sound.source.sound_1_12 = "http://static.wanzj.cn/music/voice/sound_1_12.mp3";
sound.source.sound_1_2 = "http://static.wanzj.cn/music/voice/sound_1_2.mp3";
sound.source.sound_1_3 = "http://static.wanzj.cn/music/voice/sound_1_3.mp3";
sound.source.sound_1_4 = "http://static.wanzj.cn/music/voice/sound_1_4.mp3";
sound.source.sound_1_5 = "http://static.wanzj.cn/music/voice/sound_1_5.mp3";
sound.source.sound_1_6 = "http://static.wanzj.cn/music/voice/sound_1_6.mp3";
sound.source.sound_1_7 = "http://static.wanzj.cn/music/voice/sound_1_7.mp3";
sound.source.sound_1_8 = "http://static.wanzj.cn/music/voice/sound_1_8.mp3";
sound.source.sound_1_9 = "http://static.wanzj.cn/music/voice/sound_1_9.mp3";
sound.source.sound_2_0 = "http://static.wanzj.cn/music/voice/sound_2_0.mp3";
sound.source.sound_2_1 = "http://static.wanzj.cn/music/voice/sound_2_1.mp3";
sound.source.sound_2_10 = "http://static.wanzj.cn/music/voice/sound_2_10.mp3";
sound.source.sound_2_11 = "http://static.wanzj.cn/music/voice/sound_2_11.mp3";
sound.source.sound_2_12 = "http://static.wanzj.cn/music/voice/sound_2_12.mp3";
sound.source.sound_2_2 = "http://static.wanzj.cn/music/voice/sound_2_2.mp3";
sound.source.sound_2_3 = "http://static.wanzj.cn/music/voice/sound_2_3.mp3";
sound.source.sound_2_4 = "http://static.wanzj.cn/music/voice/sound_2_4.mp3";
sound.source.sound_2_5 = "http://static.wanzj.cn/music/voice/sound_2_5.mp3";
sound.source.sound_2_6 = "http://static.wanzj.cn/music/voice/sound_2_6.mp3";
sound.source.sound_2_7 = "http://static.wanzj.cn/music/voice/sound_2_7.mp3";
sound.source.sound_2_8 = "http://static.wanzj.cn/music/voice/sound_2_8.mp3";
sound.source.sound_2_9 = "http://static.wanzj.cn/music/voice/sound_2_9.mp3";
sound.source.bgm = "http://static.wanzj.cn/music/sixoxen/bgm.mp3";
sound.source.sound_101 = "http://static.wanzj.cn/music/sixoxen/sound_101.mp3";
sound.source.sound_1_60 = "http://static.wanzj.cn/music/sixoxen/sound_1_60.mp3";
sound.source.sound_1_61 = "http://static.wanzj.cn/music/sixoxen/sound_1_61.mp3";
sound.source.sound_1_62 = "http://static.wanzj.cn/music/sixoxen/sound_1_62.mp3";
sound.source.sound_1_63 = "http://static.wanzj.cn/music/sixoxen/sound_1_63.mp3";
sound.source.sound_1_64 = "http://static.wanzj.cn/music/sixoxen/sound_1_64.mp3";
sound.source.sound_1_65 = "http://static.wanzj.cn/music/sixoxen/sound_1_65.mp3";
sound.source.sound_1_66 = "http://static.wanzj.cn/music/sixoxen/sound_1_66.mp3";
sound.source.sound_1_67 = "http://static.wanzj.cn/music/sixoxen/sound_1_67.mp3";
sound.source.sound_1_68 = "http://static.wanzj.cn/music/sixoxen/sound_1_68.mp3";
sound.source.sound_1_69 = "http://static.wanzj.cn/music/sixoxen/sound_1_69.mp3";
sound.source.sound_1_70 = "http://static.wanzj.cn/music/sixoxen/sound_1_70.mp3";
sound.source.sound_1_71 = "http://static.wanzj.cn/music/sixoxen/sound_1_71.mp3";
sound.source.sound_1_72 = "http://static.wanzj.cn/music/sixoxen/sound_1_72.mp3";
sound.source.sound_1_73 = "http://static.wanzj.cn/music/sixoxen/sound_1_73.mp3";
sound.source.sound_1_98 = "http://static.wanzj.cn/music/sixoxen/sound_1_98.mp3";
sound.source.sound_1_99 = "http://static.wanzj.cn/music/sixoxen/sound_1_99.mp3";
sound.source.sound_1x1 = "http://static.wanzj.cn/music/sixoxen/sound_1x1.mp3";
sound.source.sound_1x10 = "http://static.wanzj.cn/music/sixoxen/sound_1x10.mp3";
sound.source.sound_1x2 = "http://static.wanzj.cn/music/sixoxen/sound_1x2.mp3";
sound.source.sound_1x3 = "http://static.wanzj.cn/music/sixoxen/sound_1x3.mp3";
sound.source.sound_1x4 = "http://static.wanzj.cn/music/sixoxen/sound_1x4.mp3";
sound.source.sound_1x5 = "http://static.wanzj.cn/music/sixoxen/sound_1x5.mp3";
sound.source.sound_2_60 = "http://static.wanzj.cn/music/sixoxen/sound_2_60.mp3";
sound.source.sound_2_61 = "http://static.wanzj.cn/music/sixoxen/sound_2_61.mp3";
sound.source.sound_2_62 = "http://static.wanzj.cn/music/sixoxen/sound_2_62.mp3";
sound.source.sound_2_63 = "http://static.wanzj.cn/music/sixoxen/sound_2_63.mp3";
sound.source.sound_2_64 = "http://static.wanzj.cn/music/sixoxen/sound_2_64.mp3";
sound.source.sound_2_65 = "http://static.wanzj.cn/music/sixoxen/sound_2_65.mp3";
sound.source.sound_2_66 = "http://static.wanzj.cn/music/sixoxen/sound_2_66.mp3";
sound.source.sound_2_67 = "http://static.wanzj.cn/music/sixoxen/sound_2_67.mp3";
sound.source.sound_2_68 = "http://static.wanzj.cn/music/sixoxen/sound_2_68.mp3";
sound.source.sound_2_69 = "http://static.wanzj.cn/music/sixoxen/sound_2_69.mp3";
sound.source.sound_2_70 = "http://static.wanzj.cn/music/sixoxen/sound_2_70.mp3";
sound.source.sound_2_71 = "http://static.wanzj.cn/music/sixoxen/sound_2_71.mp3";
sound.source.sound_2_72 = "http://static.wanzj.cn/music/sixoxen/sound_2_72.mp3";
sound.source.sound_2_73 = "http://static.wanzj.cn/music/sixoxen/sound_2_73.mp3";
sound.source.sound_2_98 = "http://static.wanzj.cn/music/sixoxen/sound_2_98.mp3";
sound.source.sound_2_99 = "http://static.wanzj.cn/music/sixoxen/sound_2_99.mp3";
sound.source.sound_2x1 = "http://static.wanzj.cn/music/sixoxen/sound_2x1.mp3";
sound.source.sound_2x10 = "http://static.wanzj.cn/music/sixoxen/sound_2x10.mp3";
sound.source.sound_2x2 = "http://static.wanzj.cn/music/sixoxen/sound_2x2.mp3";
sound.source.sound_2x3 = "http://static.wanzj.cn/music/sixoxen/sound_2x3.mp3";
sound.source.sound_2x4 = "http://static.wanzj.cn/music/sixoxen/sound_2x4.mp3";
sound.source.sound_2x5 = "http://static.wanzj.cn/music/sixoxen/sound_2x5.mp3";
</script>
